...
const UpdLifeCycle = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await axios
      .put("http://localhost:5080/materials/updatelifecycleinfo", fdata)
      .then((res) => {
  ...
  useEffect(() => {
    const fetchHandler = async () => {
      await axios
        .get(
          `http://localhost:5080/materials/${materialId}/${lid}/getlifecycle`
        )
        .then((res) => {
          console.log("hhhhhhhhh");
          console.log("res.data---" + res.data);
          let params = JSON.stringify(res.data);
          let s = params.replaceAll("[", "").replaceAll("]", "");
          console.log(s);
          let s1 = JSON.parse(s);
          console.log(s1.resultdata.lifecycleno);
          setInputs(s1.resultdata);
        });
    };
    fetchHandler();
  }, [lid]);
